...
const filterButton = [
  `${translate('Lịch sử học')}`,
  `${translate('Thành tích')}`,
];

class UserClassInfoScreen extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
  ...
  render() {
    ...
    const user = navigator.getParam('user', {});
    return (
      <FlexContainer style={styles.container}>
        ...
        <ScrollView
          ...
          <View style={styles.tabContainer}>
            ...
            />
            {selected === filterButton[0] && (
              <ProgressUserLearning user={user} />
            )}
            {selected === filterButton[1] && this.renderAchievements()}
          </View>
        </ScrollView>
      </FlexContainer>
    );
...
export default connect()(UserClassInfoScreen);
