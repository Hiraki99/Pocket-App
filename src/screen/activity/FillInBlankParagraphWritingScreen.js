...
class FillInBlankParagraphWritingScreen extends React.PureComponent {
  constructor(props) {
    super(props);
  ...
  checkScriptType = () => {
    ...
    if (
      currentScriptItem &&
      currentScriptItem.type === 'fill_in_blank_paragraph_writing'
    ) {
      return true;
    }
  ...
  onSelected = (index) => {
    if (this.writingRef) {
      ...
      this.writingRef.show(this.state.answers[index]);
    }
    playAudio('selected');
  };
  ...
  getCount = () => {
    ...
    correctAnswers.forEach((item, index) => {
      const listCorrectAnswer = item.text.toLowerCase().split('/');
      if (listCorrectAnswer.includes((answers[index] || '').toLowerCase())) {
...
export default connect(mapStateToProps, {
  setMaxCorrect,
  increaseScore,
})(FillInBlankParagraphWritingScreen);
